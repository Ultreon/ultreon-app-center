...
pub fn get_version_req(cfg: &AppConfig) -> Result<VersionReq, Error> {
...
pub fn get_classpath(cfg: &AppConfig, meta: AppMetadata, data_dir: &String) -> Vec<String> {
    let mut cp = vec![];
    for entry in cfg.classpath.iter() {
        cp.push(data_dir.to_string() + "/" + entry)
    }

    cp.push(format!("{}/apps/{}/versions/{}/{}.jar", data_dir, cfg.app, meta.version, meta.version));
    cp
}
...
pub enum Error {
    ...
    Download(String),
    /// Failed to launch a app version.
...
pub fn get_data_dir() -> PathBuf {
    ...
    }.map(|it| it.join("UltreonAppCenter")).unwrap()
}
